...
public class Porpoise extends Agent {
		...
		if (!useExpFoodVal) {
			...
			if (this.ignoreDeterrence <= 0) {
				totalDX = getDx() * crwContrib + this.vt[0] + this.deterVt[0]
						* SimulationParameters.getDeterrenceCoeff();
	...
	public void deter(final double currentDeterenceStrength, final NdPoint srcPosition) {
		...
		if (this.deterStrength < currentDeterenceStrength) {
			...
			// vector pointing away from srcPosition //this is altered to account for condition-mediated response note 10 here is Ep_max/2 be cautious if Ep_max changes
			this.deterVt[0] = (currentDeterenceStrength/(1+Math.exp(-(1.24*this.energyLevel-10)))) * ((porpPosition.getX() - srcPosition.getX()));
			this.deterVt[1] = (currentDeterenceStrength/(1+Math.exp(-(1.24*this.energyLevel-10)))) * ((porpPosition.getY() - srcPosition.getY()));
	...
	public int getUtmX() {
		return (int) Math.round(getPosition().getX() * 400 + Globals.getXllCorner());
	}

	public int getUtmY() {
		return (int) Math.round(getPosition().getY() * 400 + Globals.getYllCorner());
	}
